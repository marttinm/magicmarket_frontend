...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (username === "alphaTester" && password === "alphaTester123") {
      // Simular una llamada a la API para establecer una cookie de autenticación
      ...
      // Also set regular auth for navigation
      await fetch("/api/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: "alphaTester", password: "alphaTester123" }),
      })
      ...
      <main className="flex-1 flex items-center justify-center p-6 relative">
        <div className="w-full max-w-md space-y-8">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Acceso Alpha Tester</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="mt-8 space-y-6">
            ...
            <div className="mt-4 text-center">
              <p className="text-xs text-muted-foreground">
                Admin: admin / nimda | Alpha Tester: alphaTester / alphaTester123
              </p>
            </div>
          </form>
        </div>
